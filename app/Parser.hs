...
import qualified Data.ByteString               as BS (ByteString, drop, length,
                                                      take)
...
newtype Parser a = P { unP :: BS.ByteString -> ParserOut a }
...
word32 =
    ensure 4 >> P (\i -> Done (BS.drop 4 i) (word32le i))

skip :: Int -> Parser ()
skip n =
    ensure n >> P (\i -> Done (BS.drop n i) ())

bytes :: Int -> Parser BS.ByteString
bytes n =
    ensure n >> P (\i -> Done (BS.drop n i) (BS.take n i))
