...
import qualified Data.ByteString.Lazy      as L (ByteString, drop, foldrChunks,
                                                 readFile)
...
main = do
    (fn:_) <- getArgs
    lbs <- L.readFile fn
    evalStateT (parseAndPrintChunks (L.drop pcapGlobalHdrLen lbs)) ""

parseAndPrintChunks :: L.ByteString -> StateT BS.ByteString IO ()
parseAndPrintChunks lbs =
    L.foldrChunks
        (\e a -> do
            leftover <- get
            put =<< liftIO (parseAndPrintChunk (leftover `BS.append` e))
            a)
        (return ())
        lbs
...
parseAndPrintChunk :: BS.ByteString -> IO BS.ByteString
parseAndPrintChunk chunk =
    if BS.length chunk < pcapPktHdrLen then
        ...
        let pktTime = (getWord32At 0 chunk, getWord32At 4 chunk)
            ...
            goNextPkt = parseAndPrintChunk (BS.drop (pcapPktHdrLen + pktLen) chunk)
