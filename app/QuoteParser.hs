...
parseAcceptTime inp = 
  Time {t_hours = readInt $ BS.take 2 inp
...
parseNQtyPrice
  :: Int -> BS.ByteString -> [QtyPrice]
parseNQtyPrice = go []
  where go acc 0 _ = acc
        go acc n inp = 
          go ((dropTake 5 7 inp,BS.take 5 inp) : acc)
             (n - 1)
             (BS.drop 12 inp)

parseQuotePkt
  :: Time -> BS.ByteString -> QuotePkt
parseQuotePkt inPktTime rawPkt = 
  QuotePkt {pktTime = inPktTime
...
printQuotePkt QuotePkt{..} = 
  ...
  BS.intercalate " "
                 (map qtyPriceStr bids) <>
  " " <>
  BS.intercalate " "
                 (map qtyPriceStr $ reverse asks)
  where qtyPriceStr (q,p) = padQty (rlz q) <> "@" <> padPrice (rlz p)
        rlz a = 
        ...
        padQty a = 
          BS.replicate (7 - BS.length a)
                       0x20 <>
          a
        padPrice a = 
          a <>
          BS.replicate (5 - BS.length a)
                       0x20
